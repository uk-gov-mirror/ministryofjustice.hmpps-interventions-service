...
class DraftReferralTest : IntegrationTestBase() {
  ...
  @Test
  fun `create draft referral dto from empty referral`() {
  ...
  @Test
  fun `create draft referral dto with empty referral id`() {
    val createdDate = SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse("2020-12-04 10:42:43")
  ...
  @Test
  fun `create draft referral dto with empty referral creation date`() {
  ...
  fun `determine json for draft referral dto`() {
    ...
    val draftReferral = DraftReferral(Referral(id = UUID.fromString(uuid), created = SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse("2020-12-04 10:42:43")))
